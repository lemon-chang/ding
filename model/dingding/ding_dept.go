...
func (d *DingDept) ImportDeptData() (DepartmentList []DingDept, err error) {
	...
	//	tx.Select(clause.Associations).Delete(&Deleted)
	//	//根据部门id存储一下部门用户
	...
	//			tempUserList, hasMore, err = d.GetUserListByDepartmentID(0, 50)
	//			if err != nil {
	...
	//		err = tx.Select(clause.Associations).Delete(&userDeleted).Error
	//		err = tx.Clauses(clause.OnConflict{
	//			Columns:   []clause.Column{{Name: "user_id"}},
	//			DoUpdates: clause.AssignmentColumns([]string{"name", "title"}),
	//		}).Create(&UserList).Error
	//
	//		err = tx.Model(&DepartmentList[i]).Association("UserList").Replace(UserList)
	//	}
	//	return err
	//})
	//return
